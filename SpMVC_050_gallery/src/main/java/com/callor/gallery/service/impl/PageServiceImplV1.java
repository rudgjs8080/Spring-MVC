...
public class PageServiceImplV1 implements PageService{
	...
	public PageDTO makePaination(int totalListSize, int currentPage) {
		...
		/*
		 * 이 연산을 수행하면 전체 리스트를 표현하는데
		 * 몇페이지가 필요한지를 계산하는 코드
		 * 
		 * 이 연산은 int 형 데이터로 연산을 수행하므로 소수점이하를
		 * 무조건 자른다
		 * 마지막 페이지 개수가 listPerPage 보다 작으면 
		 * 마지막 페이지는 무시하는
		 * totalPage값이 계산된다
		 */
