...
public class MyController {
	 ...
	 * 		DELETE : 삭제(DELETE)
	 * 
	 * RequestMapping이 같고, RequestMethod가 같은 클래스는
	 * 절대 허용하지 않는다
	 * 
	 ...
	 * 요청을 받으면
	 * @RequestMapping(value="/insert", method=GET)을 처리하는
	 * method가 있어야 한다
	 * 
	...
	/*
	 * Spring 에서는 Web으로부터 전달받은 데이터를
	 * Java method의 매개변수를 설정하여 받을 수 있다
	 * 
	 * Spring framework 내부에서 Reflection이라는 코드가 작동되어서
	 * 전달받은 변수이름과 Java method 매개변수와 이름매칭 방법으로
	 * 변수를 받는다
	 * 
	 * 이런 코드는 개발자가 신경쓰지않아도 자동으로 작동되는 코드이다
	 ...
	 * Reflection 코드가 중단되고
	 * Spring 의 Dispatcher는 Web 400 Status Code를
	 * 응답해 버린다
	 * 
	...
	public void insert(String test, int number) {
		
		Integer.valueOf(test);
		
		/*String strNum = "300A";
		// NumberformatException이 나올 것 
